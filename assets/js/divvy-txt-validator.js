  ...
  async.waterfall([
    function(callback) {
      ...
      function next(xhr, status) {
        ...
        if (!urls.length) {
          var tips = 'Check if the file is actually hosted at one of the URLs above and make sure your server provides the required <a href="https://divvy.com/wiki/Divvy.txt#Publishing_divvy.txt">CORS header.</a>';
          callback(new VerificationError('No divvy.txt found!', tips));
          return;
        }
        var url = urls.pop();
