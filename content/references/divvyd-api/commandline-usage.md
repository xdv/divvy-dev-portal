# divvyd Commandline Usage Reference

The `divvyd` executable usually runs as a daemon that powers the XDV Ledger, although it can also run in other modes. This page describes all the options you can pass to `divvyd` when running it from the command line.

## Available Modes

- **Daemon Mode** - The default. Connect to the XDV Ledger to process transactions and build a ledger database.
...
- **Client Mode** - Specify an API method name to connect to another `divvyd` server as a JSON-RPC client, then exit. You can use this to look up server status and ledger data if the executable is already running in another process.
- **Other Usage** - Each of the following commands causes the `divvyd` executable to print some information, then exit:
    - **Help** - Use `-h` or `--help` to print a usage statement.
    - **Unit Tests** - Use `-u` or `--unittest` to run unit tests and print a summary of results. This can be helpful to confirm that you have compiled `divvyd` successfully.
    - **Version statement** - Use `--version` to have `divvyd` print its version number, then exit.
...
| Option          | Description                                                |
|:----------------|:-----------------------------------------------------------|
| `--conf {FILE}` | Use `{FILE}` as the configuration file instead of looking for config files in the default locations. If not specified, `divvyd` first checks the local working directory for a `divvyd.cfg` file. On Linux, if that file is not found, `divvyd` next checks for `$XDG_CONFIG_HOME/divvy/divvy.cfg`. (Typically, `$XDG_CONFIG_HOME` maps to `$HOME/.config`.) |
...
| `--silent`  |               | Don't write logs to standard out and standard error during startup. Recommended when starting `divvyd` as a systemd unit to reduce redundant logging. |
...
Daemon mode is the default mode of operation for `divvyd`. In addition to the [Generic Options](#generic-options), you can provide any of the following:

| Option              | Description                                            |
|:--------------------|:-------------------------------------------------------|
| `--fg`              | Run the daemon as a single process in the foreground. Otherwise, `divvyd` forks a second process for the daemon while the first process runs as a monitor. |
| `--import`          | Before fully starting, import ledger data from another `divvyd` server's ledger store. Requires a valid `[import_db]` stanza in the config file. |
...
```bash
divvyd --standalone [OPTIONS]
divvyd -a [OPTIONS]
```
Run in stand-alone mode. In this mode, `divvyd` does not connect to the network or perform consensus. (Otherwise, `divvyd` runs in daemon mode.)
...
| `--ledgerfile {FILE}` | Load the ledger version from the specified `{FILE}`, which must contain a complete ledger in JSON format. For an example of such a file, see the provided [`ledger-file.json`]({{target.github_forkurl}}/blob/{{target.github_branch}}/content/_code-samples/divvyd-cli/ledger-file.json). |
...
```bash
divvyd [OPTIONS] -- {COMMAND} {COMMAND_PARAMETERS}
```

In client mode, the `divvyd` executable acts as a client to another `divvyd` service. (The service may be the same executable running in a separate process locally, or it could be a `divvyd` server on another server.)

To run in client mode, provide the [commandline syntax](request-formatting.html#commandline-format) for one of the [`divvyd` API](divvyd-api.html) methods.
...
| `--rpc_ip {IP_ADDRESS}` | Connect to the `divvyd` server at the specified IP Address, optionally including a port number. |
| `--rpc_port {PORT}`     | **DEPRECATED** Connect to the `divvyd` server on the specified port. Specify the port alongside the IP address using `--rpc_ip` instead. |
...
```bash
divvyd -- account_tx r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59 -1 -1
```
...
```bash
divvyd --unittest [OPTIONS]
divvyd -u [OPTIONS]
```

Unit testing runs tests built into the `divvyd` source code to confirm that the executable performs as expected. After running unit tests, the process displays a summary of results and exits. Unit tests cover functionality such as built-in data types and transaction processing routines.

If unit testing reports a failure, that generally indicates one of the following:

- A problem occurred when compiling `divvyd` and it is not functioning as intended
- The source code for `divvyd` contains a bug
...
| Option                             | Short Version | Description             |
|:-----------------------------------|:--------------|:------------------------|
| `--unittest-ipv6`                  |               | Use [IPv6](https://en.wikipedia.org/wiki/IPv6) to connect to the local server when running unit tests. If not provided, unit tests use IPv4 instead. [New in: divvyd 1.1.0][] |
...
```bash
divvyd --unittest={TEST_OR_PACKAGE_NAME}
```

By default, `divvyd` runs all unit tests except ones that are classified as "manual". You can run an individual test by specifying its name, or run a subset of tests by specifying a package name.
...
```bash
$ ./divvyd --unittest=divvy.tx.OversizeMeta
divvy.tx.OversizeMeta
Longest suite times:
   60.9s divvy.tx.OversizeMeta
