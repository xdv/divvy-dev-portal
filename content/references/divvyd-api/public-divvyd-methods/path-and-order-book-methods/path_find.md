# path_find
[[Source]<br>](https://github.com/xdv/divvyd/blob/master/src/divvy/rpc/handlers/PathFind.cpp "Source")

*WebSocket API only!* The `path_find` method searches for a [path](paths.html) along which a transaction can possibly be made, and periodically sends updates when the path changes over time. For a simpler version that is supported by JSON-RPC, see the [divvy_path_find method][]. For payments occurring strictly in XDV, it is not necessary to find a path, because XDV can be sent directly to any account.
...
| `destination_amount`  | String or Object | [Currency Amount][] that the destination account would receive in a transaction. **Special case:** [New in: divvyd 0.30.0][] You can specify `"-1"` (for XDV) or provide -1 as the contents of the `value` field (for non-XDV currencies). This requests a path to deliver as much as possible, while spending no more than the amount specified in `send_max` (if provided). |
| `send_max`            | String or Object | _(Optional)_ [Currency Amount][] that would be spent in the transaction. Not compatible with `source_currencies`. [New in: divvyd 0.30.0][] |
...
| `full_reply`          | Boolean          | If `false`, this is the result of an incomplete search. A later reply may have a better path. If `true`, then this is the best path found. (It is still theoretically possible that a better path could exist, but `divvyd` won't find it.) Until you close the pathfinding request, `divvyd` continues to send updates each time a new ledger closes. [New in: divvyd 0.29.0][] |
...
* `noEvents` - You are using a protocol that does not support asynchronous callbacks, for example JSON-RPC. (See the [divvy_path_find method][] for a pathfinding method that _is_ compatible with JSON-RPC.)
...
If the follow-up includes `"full_reply": true`, then this is the best path that divvyd can find as of the current ledger.
...
## path_find close
[[Source]<br>](https://github.com/xdv/divvyd/blob/master/src/divvy/rpc/handlers/PathFind.cpp#L58-L67 "Source")
...
* `noEvents` - If you tried to use this method on a protocol that does not support asynchronous callbacks, for example JSON-RPC. (See the [divvy_path_find method][] for a pathfinding method that _is_ compatible with JSON-RPC.)
* `noPathRequest` - You tried to close a pathfinding request when there is not an open one.

## path_find status
[[Source]<br>](https://github.com/xdv/divvyd/blob/master/src/divvy/rpc/handlers/PathFind.cpp#L69-L77 "Source")
