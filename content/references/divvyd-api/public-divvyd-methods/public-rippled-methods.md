# Public divvyd Methods

Communicate directly with a `divvyd` server using the following public API methods. Public methods are not necessarily meant for the general public, but they are used by any client attached to the server. Think of public methods as being for members or customers of the organization running the server.


## [Account Methods](account-methods.html)

An account in the XDV Ledger represents a holder of XDV and a sender of transactions. Use these methods to work with account info.
...
* **[`nodivvy_check`](nodivvy_check.html)** - Get recommended changes to an account's DefaultDivvy and NoDivvy settings.
...
Transactions are the only thing that can modify the shared state of the XDV Ledger. All business on the XDV Ledger takes the form of transactions. Use these methods to work with transactions.
...
* **[`divvy_path_find`](divvy_path_find.html)** - Find a path for payment between two accounts, once.
...
Use these methods to retrieve information about the current state of the `divvyd` server.
