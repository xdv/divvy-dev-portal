# subscribe
[[Source]<br>](https://github.com/xdv/divvyd/blob/master/src/divvy/rpc/handlers/Subscribe.cpp "Source")
...
```
{
    "id": "Example subscribe to XDV/GateHub USD order book",
    "command": "subscribe",
    "books": [
        {
            "taker_pays": {
...
* `server` - Sends a message whenever the status of the `divvyd` server (for example, network connectivity) changes
...
* `validations` - Sends a message whenever the server receives a validation message, regardless of if the server trusts the validator. (An individual `divvyd` declares a ledger validated when the server receives validation messages from at least a quorum of trusted validators.)
* `peer_status` - **(Admin only)** Information about connected peer `divvyd` servers, especially with regards to the consensus process.
...
* *Stream: ledger* - Information about the ledgers on hand and current fee schedule, such as `fee_base` (current base fee for transactions in XDV), `fee_ref` (current base fee for transactions in fee units), `ledger_hash` (hash of the latest validated ledger), `reserve_base` (minimum reserve for accounts), and more.
...
* `malformedAccount` - One of the addresses in the `accounts` or `accounts_proposed` fields of the request is not a properly-formatted XDV Ledger address. (**Note:**: You _can_ subscribe to the stream of an address that does not yet have an entry in the global ledger to get a message when that address becomes funded.)
...
The `ledger` stream only sends `ledgerClosed` messages when [the consensus process](https://xdv.io/build/divvy-ledger-consensus-process/) declares a new validated ledger. The message identifies the ledger and provides some information about its contents.
...
| `fee_base`          | Unsigned Integer | Cost of the 'reference transaction' in drops of XDV. (See [Transaction Cost](transaction-cost.html) If the ledger includes a [SetFee pseudo-transaction](setfee.html) the new transaction cost applies to all transactions after this ledger. |
...
| `ledger_time`       | Unsigned Integer | The time this ledger was closed, in [seconds since the Divvy Epoch][] |
| `reserve_base`      | Unsigned Integer | The minimum reserve, in drops of XDV, that is required for an account. If the ledger includes a [SetFee pseudo-transaction](setfee.html) the new base reserve applies after this ledger. |
...
| `amendments`            | Array of Strings | (May be omitted) The [amendments](amendments.html) this server wants to be added to the protocol. [New in: divvyd 0.32.0][] |
| `base_fee`              | Integer          | (May be omitted) The unscaled transaction cost (`reference_fee` value) this server wants to set by [Fee Voting](fee-voting.html). [New in: divvyd 0.32.0][] |
| `flags`                 | Number           | Bit-mask of flags added to this validation message. The flag 0x80000000 indicates that the validation signature is fully-canonical. The flag 0x00000001 indicates that this is a full validation; otherwise it's a partial validation. Partial validations are not meant to vote for any particular ledger. A partial validation indicates that the validator is still online but not keeping up with consensus. [New in: divvyd 0.32.0][] |
| `full`                  | Boolean          | If `true`, this is a full validation. Otherwise, this is a partial validation. Partial validations are not meant to vote for any particular ledger. A partial validation indicates that the validator is still online but not keeping up with consensus. [New in: divvyd 0.32.0][] |
| `ledger_hash`           | String           | The identifying hash of the proposed ledger is being validated. |
| `ledger_index`          | String - Integer | The [Ledger Index][] of the proposed ledger. [New in: divvyd 0.31.0][] |
| `load_fee`              | Integer          | (May be omitted) The local load-scaled transaction cost this validator is currently enforcing, in fee units. [New in: divvyd 0.32.0][] |
| `reserve_base`          | Integer          | (May be omitted) The minimum reserve requirement (`account_reserve` value) this validator wants to set by [Fee Voting](fee-voting.html). [New in: divvyd 0.32.0][] |
| `reserve_inc`           | Integer          | (May be omitted) The increment in the reserve requirement (`owner_reserve` value) this validator wants to set by [Fee Voting](fee-voting.html). [New in: divvyd 0.32.0][] |
| `signature`             | String           | The signature that the validator used to sign its vote for this ledger. |
| `signing_time`          | Number           | When this validation vote was signed, in [seconds since the Divvy Epoch][]. [New in: divvyd 0.32.0][] |
...
The admin-only `peer_status` stream reports a large amount of information on the activities of other `divvyd` servers to which this server is connected, in particular their status in the consensus process.
...
Peer Status stream messages represent some event where the status of the peer `divvyd` server changed. These messages are JSON objects with the following fields:
...
| `date`             | Number | The time this event occurred, in [seconds since the Divvy Epoch][]. |
