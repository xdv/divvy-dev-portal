...
[[Source]<br>](https://github.com/xdv/divvyd/blob/f65cea66ef99b1de149c02c15f06de6c61abf360/src/divvy/app/transactors/SetAccount.cpp "Source")

An AccountSet transaction modifies the properties of an [account in the XDV Ledger](accountroot.html).
...
You can put any domain in your account's `Domain` field. To prove that an account and domain belong to the same person or business, Divvy recommends establishing a "two-way link":

- Accounts you own should have a domain you own in the `Domain` field.
- On a website at that domain, host a text file listing accounts you own, and optionally other information about how you use the XDV Ledger. Conventionally, this file is named `divvy.txt`. For an example, see <https://xdv.io/divvy.txt>.
...
| asfDefaultDivvy | 8             | lsfDefaultDivvy          | Enable [divvying](divvying.html) on this account's trust lines by default. [New in: divvyd 0.27.3][] |
...
| asfDisallowXDV   | 3             | lsfDisallowXDV            | XDV should not be sent to this account. (Enforced by client applications, not by `divvyd`) |
...
| tfDisallowXDV     | 0x00100000 | 1048576       | asfDisallowXDV (SetFlag)    |
| tfAllowXDV        | 0x00200000 | 2097152       | asfDisallowXDV (ClearFlag)  |
...
Incoming transactions with unclear purposes may be an inconvenience for financial institutions, who would have to recognize when a customer made a mistake, and then potentially refund accounts or adjust balances depending on the mistake. The `asfRequireDest` and `asfDisallowXDV` flags are intended to protect users from accidentally sending funds in a way that is unclear about the reason the funds were sent.
...
You can protect against unwanted incoming payments for non-XDV currencies by not creating trust lines in those currencies. Since XDV does not require trust, the `asfDisallowXDV` flag is used to discourage users from sending XDV to an account. However, this flag is not enforced in `divvyd` because it could potentially cause accounts to become unusable. (If an account did not have enough XDV to send a transaction that disabled the flag, the account would be completely unusable.) Instead, client applications should disallow or discourage XDV payments to accounts with the `asfDisallowXDV` flag enabled.
