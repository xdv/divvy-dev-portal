# CheckCash
[[Source]<br>](https://github.com/xdv/divvyd/blob/master/src/divvy/app/tx/impl/CashCheck.cpp "Source") <!--{# TODO: change from develop to master when 0.90.0 is released #}-->
...
- If the `Amount` or `DeliverMin` does not match the currency (and issuer, if not XDV) of the Check, the transaction fails with the result `temBAD_CURRENCY`.
