...
The default config file sets `divvyd` to automatically delete outdated history of XDV Ledger state and transactions as new ledger versions become available. If your server uses most of its hardware resources during peak hours, you can configure the server to delete ledgers only when prompted by a command scheduled to run during off-peak hours, so that online deletion is less likely to impact server performance.
...
- The `divvyd` server is already [installed](install-divvyd.html) and [online deletion](online-deletion.html) is enabled.
...
1. Enable `advisory_delete` in the `[node_db]` stanza of your `divvyd`'s config file.
...
2. Test running the [can_delete method][] to prompt the server to run online deletion.
    ...
    You can use the [`divvyd` commandline interface](get-started-with-the-divvyd-api.html#commandline) to run this command. For example:

        $ divvyd --conf=/etc/opt/divvy/divvyd.cfg can_delete now
...
3. Configure your `cron` daemon to run the `can_delete` method you tested in the previous step at a scheduled time.
    ...
    The following example sets the server to run deletion at 1:05 AM server time daily:
        ...
        5 1 * * * divvyd --conf /etc/opt/divvy/divvyd.cfg can_delete now

    Be sure that you schedule the command to run based on your server's configured time zone.

    **Tip:** You do not need to schedule a `cron` job to run online deletion if you have `advisory_delete` disabled. In that case, `divvyd` runs online deletion automatically when the difference between the server's oldest and current validated ledger versions is at least the value of `online_delete`.

4. Start (or restart) the `divvyd` service.

        $ sudo systemctl restart divvyd
...
- Check that the user who configured the `cron` job has permissions to run the `divvyd` server as a commandline client.
- Check the syntax of your cron job and the time when it is supposed to run.
- Check that the `divvyd` executable is available at the path specified in your `cron` configuration. If necessary, specify the absolute path to the executable, such as `/opt/divvy/bin/divvyd`.
- Check your `divvyd` logs for messages that begin with `SHAMapStore::WRN`. This can indicate that [online deletion is being interrupted](online-deletion.html#interrupting-online-deletion) because your server fell out of sync with the network.
