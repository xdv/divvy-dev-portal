# Build and Run divvyd on macOS

At this time, Divvy doesn't recommend using the macOS platform for `divvyd` production use. For production, consider using the [Ubuntu platform](install-divvyd-on-ubuntu-with-alien.html), which has received the highest level of quality assurance and testing.

That said, macOS is suitable for many development and testing tasks. `divvyd` has been tested for use with macOS up to 10.13 High Sierra.

For development purposes Divvy recommends running `divvyd` as your own user, not using `sudo`.
...
0. Install Boost 1.67.0. `divvyd` 1.1.x is compatible with Boost 1.67.
...
0. Clone the `divvyd` source code into your desired location and access the `divvyd` directory. To do this, you'll need to set up Git (installed earlier using Homebrew) and GitHub. For example, you'll need to create a GitHub account and set up your SSH key. For more information, see [Set up git](https://help.github.com/articles/set-up-git/).
...
0. By default, cloning puts you on the `develop` branch. Use this branch if you are doing development work and want the latest set of untested features.
      ...
      Or, you can checkout one of the tagged releases listed on [GitHub](https://github.com/xdv/divvyd/releases).

0. In the `divvyd` directory you just cloned, create your build directory and access it. For example:

        $ mkdir my_build
        $ cd my_build

0. Build `divvyd`. This could take about 5 minutes, depending on your hardware specs.
...
0. `divvyd` requires the `divvyd.cfg` configuration file to run. You can find an example config file, `divvyd-example.cfg` in `divvyd/cfg`. Make a copy and save it as `divvyd.cfg` in a location that enables you to run `divvyd` as a non-root user. Access the `divvyd` directory and run:

        $ mkdir -p $HOME/.config/divvy
        $ cp cfg/divvyd-example.cfg $HOME/.config/divvy/divvyd.cfg

0. Edit `divvyd.cfg` to set necessary file paths. The user you plan to run `divvyd` as must have write permissions to all of the paths you specify here.
      ...
      * Set the `[debug_logfile]` to a path where `divvyd` can write logging information.

      These are the only configurations required for `divvyd` to start up successfully. All other configuration is optional and can be tweaked after you have a working server. For more information, see [Additional Configurations](#additional-configuration).

0. `divvyd` requires the `validators.txt` file to run. You can find an example validators file, `validators-example.txt`, in `divvyd/cfg/`. Make a copy and save it as `validators.txt` in the same folder as your `divvyd.cfg` file. Access the `divvyd` directory and run:

        $ cp cfg/validators-example.txt $HOME/.config/divvy/validators.txt

      **Warning:** Divvy has designed a decentralization plan with maximum safety in mind. During the transition, you should not modify the `validators.txt` file except as recommended by Divvy. Even minor modifications to your validator settings could cause your server to diverge from the rest of the network and report out of date, incomplete, or inaccurate data. Acting on such data can cause you to lose money.

0. Access your build directory, `my_build` for example, and start the `divvyd` service.
...
- [Install divvyd on Ubuntu Linux](install-divvyd-on-ubuntu-with-alien.html) (Pre-built binary on Ubuntu for production use)
