...
Financial institutions and other services using the XDV Ledger should use the best practices described here to make sure that transactions are validated or rejected in a verifiable and prompt way.  You should submit transactions to trusted (locally operated) `divvyd` servers.

The best practices detailed in this document allow applications to submit transactions to the XDV Ledger while achieving:
...
The XDV Ledger protocol provides a ledger shared across all servers in the network.  Through a [process of consensus and validation](https://xdv.io/build/divvy-ledger-consensus-process/), the network agrees on the order in which transactions are applied to (or omitted from) the ledger.

Well-formed transactions submitted to trusted XDV Ledger servers are usually validated or rejected in a matter of seconds.  There are cases, however, in which a well-formed transaction is neither validated nor rejected this quickly. One specific case can occur if the global [transaction cost](transaction-cost.html) increases after an application sends a transaction.  If the transaction cost increases above what has been specified in the transaction, the transaction is not included in the next validated ledger. If at some later date the global transaction cost decreases, the transaction could be included in a later ledger. If the transaction does not specify an expiration, there is no limit to how much later this can occur.
...
The XDV Ledger provides several APIs for submitting transactions, including [`divvyd` API](divvyd-api.html), and [DivvyAPI](divvyapi-reference.html).  Regardless of the API used, the transaction is applied to the ledger as follows.
...
**Note:** When submitting a transaction via `divvyd`, a successful status code returned from a submit command indicates the `divvyd` server has received the candidate transaction. The transaction may or may not be applied to a validated ledger.
...
While applying transactions, `divvyd` servers use the *last validated ledger*, a snapshot of the ledger state based on transactions the entire network has validated.  The process of consensus and validation apply a set of new transactions to the last validated ledger in canonical order, resulting in a new validated ledger.  This new validated ledger instance and the ones that preceded it form the ledger history.
...
Applications using `divvyd` APIs should explicitly specify a `LastLedgerSequence` when submitting transactions.

DivvyAPI uses the `maxLedgerVersion` field of [Transaction Instructions](divvyapi-reference.html#transaction-instructions) to specify the `LastLedgerSequence`.  DivvyAPI automatically provides an appropriate value by default.  You can specify `maxLedgerVersion` as `null` to intentionally omit `LastLedgerSequence`, in case you want a transaction that can be executed after an unlimited amount of time (this is strongly discouraged).
...
Your `divvyd` server should automatically acquire the missing ledger versions when it has spare resources (CPU/RAM/disk IO) to do so, unless the ledgers are older than its [configured amount of history to store](https://github.com/xdv/divvyd/blob/develop/cfg/divvyd-example.cfg#L581). Depending on the size of the gap and the resource usage of your server, acquiring missing ledgers should take a few minutes. You can also manually request your server to acquire historical ledger versions using the [ledger_request method][].

Alternatively, you can look up the status of the transaction using a different `divvyd` server that already has the needed ledger history, such as Divvy's full-history servers at `s2.xdv.io`. Only use a server you trust for this purpose. A malicious server could be programmed to provide false information about the status and outcome of a transaction.
...
1. The [`divvyd` API](divvyd-api.html)
2. [DivvyAPI](divvyapi-reference.html)
3. Any number of other software APIs layered on top of `divvyd`


### divvyd - Submitting and Verifying Transactions

#### Determine the Account Sequence

`divvyd` provides the [account_info method][] to learn an account's sequence number in the last validated ledger.
...
`divvyd` provides the [sign method][] to prepare a transaction for submission.  This method requires an account secret, which should only be passed to trusted `divvyd` instances.  This example issues 10 FOO (a made-up currency) to another XDV Ledger address.
...
`divvyd` provides the [submit method][], allowing us to submit the signed transaction.  This uses the `tx_blob` parameter that was returned by the `sign` method.
...
The `txnNotFound` result code occurs in cases where the transaction is not included in any ledger.  However, it could also occur when a `divvyd` instance does not have a complete ledger history, or if the transaction has not yet propagated to the `divvyd` instance.  Applications should make further queries to determine how to react.
...
Finally the server may show one or more gaps in the transaction history. The `completed_ledgers` field shown in the response above indicates that ledgers 10256383 through 10256411 are missing from this divvyd instance.  Our example transaction can only appear in ledgers 10268597 - 10268600 (based on when it was submitted and `LastLedgerSequence`), so the gap shown here is not relevant.  However, if the gap indicated a ledger in that range was missing, then an application would need to query another divvyd server (or wait for this one to retrieve the missing ledgers) to determine that a `txnNotFound` result is immutable.
...
- [Overview of XDV Ledger Consensus Process](consensus.html)
